...
def write(data="", timeout=20):
    ...
    try:          
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s: #TCP
            ...
            s.connect((ip, port))
            ret = s.send(data.encode())
    except Exception as e: 
      print(e)
    
    return ret

def query(data="", delay=0.05, timeout=20):     
    ret = 0
    print(data)
...
def move_to(el=0, azi=0, timeout=20):    
    ret = 0
    
    try:                
        data = "FXT:MOVE %d,%d\r" % (el, azi)
        print(data)    
...
'''        
def run_el_sweep(start=0, stop=0, step=1, speed=100, delay=1):              
    query("CONF:FXT:SPD %d\r" % (speed))
...
move_to(el=0, azi=0, timeout=20)
disconnect()
   

'''
#run_el_sweep(start=-60, stop=60, step=10, speed=100, delay=0.1)
query("MODEL?\r")
query("SERIAL?\r")
query("FXT:MOVE?\r")
query("FXT:RST\r")
query("FXT:TORQ ON\r")
query("FXT:TORQ OFF\r")
query("FXT:TORQ?\r")
query("READ:FXT:SPD?\r")
query("READ:IP?\r")
query("READ:GWAY?\r")
query("READ:SMASK?\r")
query("READ:PDNS?\r")
