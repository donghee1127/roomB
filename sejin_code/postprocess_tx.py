"""TX 측정 CSV -> 정리된 xlsx 후처리 (tidy/long 포맷, 피벗 가능).

RX 후처리(scripts/postprocess_rx.py)의 TX 판. out/ 의 측정 CSV(op1db /
gain_index_accuracy common·chan / evm)를 채널별로 모아 한 워크북으로 정리한다.
데이터는 Beam/Channel 컬럼을 붙인 long 포맷으로 쭉 쌓아 엑셀에서 필터·피벗으로
채널을 골라볼 수 있게 한다.

RX 와의 차이:
- linearity 소스가 ip1db(RX) 가 아니라 op1db(TX). 헤드라인 스펙도 OP1dB.
- 레일 이름이 다르지만(FE1_4V0/CORE_1V0/IO_ANA_1V8/DIST_1V8/FE2_1V8/FE3_1V8)
  `_V` 접미 컬럼 자동 감지라 코드 변경 없이 처리된다.

사용:
  python scripts/postprocess_tx.py [입력폴더] [출력.xlsx]
  기본 입력 = "out/Stampede EVB Test Results", 출력 = 그 폴더의 Stampede_TX_Test_Results.xlsx

시트:
  Summary           채널별 핵심 지표 1행
  Linearity         OP1dB sweep (레일 I[mA] + Psum[W]); OP1dB 마커
  ...
  TotalPDC          OP1dB 동작점 레일 I[mA] + Psum
...
DEFAULT_DIR = Path("out/Stampede EVB Test Results")
BOLD = Font(bold=True)
...
def rail_names(header: list[str]) -> list[str]:
    """'<rail>_V' 와 '<rail>_mA' 가 짝으로 있는 것만 레일로 본다.

    _V 로 끝나기만 하면 레일로 치면 vdd_sensitivity CSV 의 'VDD_set_V'(인가 전압
    열)까지 레일로 잡히고, 짝이 없는 'VDD_set_mA' 를 찾다 IndexError 로 죽는다.
    """
    return [h[:-2] for h in header
            if h.endswith("_V") and f"{h[:-2]}_mA" in header]


def classify(path: Path, meta: dict) -> str:
    n = path.name
    # TX linearity = op1db sweep. (ip1db 도 같은 sweep 구조라 함께 받아 둔다.)
    if n.startswith("op1db") or n.startswith("ip1db"):
        return "linearity"
...
def write_summary(ws, channels):
    ws.append(["Stampede TX Test Summary"])
    ws["A1"].font = TITLE
    ws.append([])
    hrow(ws, ["Beam", "Channel", "Freq(MHz)", "OP1dB(dBm)", "IP1dB(dBm)",
              "SmallSigGain(dB)", "CommonRange(dB)", "ChanRange(dB)",
              "BestEVM(%)", "BestEVM@SG(dBm)", "PDC@OP1dB(W)"])
    for key in sorted(channels):
        ...
        if lin:
            ...
            pdc = _pdc_at_op1db(lin)
        ws.append([beam, chan, _r(freq, 0), _r(op1), _r(ip1), _r(ssg),
...
def write_linearity(ws, channels):
    ...
    for key in sorted(channels):
        ...
        for i in range(len(lin["rows"])):
            tag = "OP1dB" if i == mark else ("interp" if i in timeout else "")
...
def write_evm_pivot(ws, channels):
    ...
    와이드 표(Pin x 채널 EVM%) + 드롭다운(채널) -> INDEX/MATCH 헬퍼 -> 라인차트.
...
def write_total_pdc(ws, channels):
    rails = _any_rails(channels, "linearity")
    hrow(ws, ["Beam", "Channel", "Pin@OP1dB(dBm)"]
...
def main(argv):
    ...
    out_path = (Path(argv[2]) if len(argv) > 2
                else in_dir / "Stampede_TX_Test_Results.xlsx")
